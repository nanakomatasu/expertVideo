...
const useColumnNoticeBar = () => {
  ...
  const data = common_vendor.computed(
    () => uni_modules_tuniaouiVue3_utils_isEmpty.isEmptyVariableInDefault(noticeBar == null ? void 0 : noticeBar.data, [])
  );
  const interval = common_vendor.computed(
    () => uni_modules_tuniaouiVue3_utils_isEmpty.isEmptyVariableInDefault(noticeBar == null ? void 0 : noticeBar.speed, 3e3)
  );
  const play = common_vendor.computed(
    () => uni_modules_tuniaouiVue3_utils_isEmpty.isEmptyVariableInDefault(noticeBar == null ? void 0 : noticeBar.play, true)
  );
  const vertical = common_vendor.computed(() => (noticeBar == null ? void 0 : noticeBar.direction) === "vertical");
  const noticeClickEvent = (index) => {
    noticeBar == null ? void 0 : noticeBar.click(index);
  };
  return {
    data,
    interval,
