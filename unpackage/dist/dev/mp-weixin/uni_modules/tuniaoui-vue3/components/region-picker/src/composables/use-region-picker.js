...
const useRegionPicker = (porps, emits) => {
  ...
  const getCodeByNames = (names) => {
    ...
    do {
      ...
      const regionItem = data.find((item) => item.name === name) || data[0];
      if (regionItem) {
        code.push(regionItem.code);
      }
      data = (regionItem == null ? void 0 : regionItem.children) || [];
    } while (data.length);
    return code;
  };
  const getNameByCodes = (codes) => {
    ...
    do {
      ...
      const regionItem = data.find((item) => item.code === code) || data[0];
      if (regionItem) {
    ...
    (val) => {
      ...
      if (val.length) {
        ...
        if (chineseReg.test(value[0])) {
          ...
          isInnerUpdate = true;
          emits(
            uni_modules_tuniaouiVue3_constants_event.UPDATE_MODEL_EVENT,
            getNameByCodes(uni_modules_tuniaouiVue3_utils_cloneDeep.cloneDeep(currentSelectValue.value))
          );
